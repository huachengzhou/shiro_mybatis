...
<body>
	...
	function doGoPage(pageNumber){
		var url = "${ctx}/showListInfo?pageNumber="+pageNumber;
		window.location.href = url;
	};
	function doGoPageN(){
		var pageNumber = document.getElementById('pageNumberN').value;
		if(pageNumber==0){
			pageNumber = 1;
		}
		var url = "${ctx}/showListInfo?pageNumber="+pageNumber;
	...
	<div>
		<ul class="pager">
			<c:if test="${page.pageNumber>1}">
			    <li><a href="javascript:doGoPage(${page.pageNumber-1})">Previous</a></li>
			</c:if>
			
		    <li><span><label class="label">
		    <font color="blue">共${page.totalPage}页  </font> 
		    </label></span> </li>
		    
		    <c:if test="${page.pageNumber<page.totalPage}">
			    <li><a href="javascript:doGoPage(${page.pageNumber+1})">Next</a></li>
		    </c:if>
		    <input type="text" id="pageNumberN" onchange="doGoPageN()"  style="height:40px;" class="form-control" size="5" placeholder="跳转到"> 
		</ul>
	</div>
</body>
</html>
